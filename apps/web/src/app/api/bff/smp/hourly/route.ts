...
const forwardedQueryParams = ['area', 'from', 'to', 'cursor', 'limit'] as const;

/**
 * 시간별 SMP BFF proxy (§10.3): session cookie → Better Auth JWT → NestJS Bearer call.
...
export async function GET(request: Request): Promise<Response> {
  ...
  const apiUrl = new URL(`${API_BASE_URL}/api/v1/market/smp/hourly`);
