...
const forwardedQueryParams = ['region', 'fuelType', 'from', 'to', 'cursor', 'limit'] as const;

/**
 * 일별 발전량 BFF proxy (§10.3): session cookie → Better Auth JWT → NestJS Bearer call.
...
export async function GET(request: Request): Promise<Response> {
  ...
  const apiUrl = new URL(`${API_BASE_URL}/api/v1/generation/hourly`);
  ...
  apiUrl.searchParams.set('bucket', 'daily');
